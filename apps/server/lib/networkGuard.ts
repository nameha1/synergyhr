...
export async function requireOfficeNetwork(
  req: Request
  ...
  if (!ip || ip === "127.0.0.1" || ip === "::1" || ip === "::ffff:127.0.0.1") {
    if (process.env.NODE_ENV === "development") {
      ...
      return { ok: true, ip: ip || "127.0.0.1", asn: 0 };
    }
    return { ok: false, reason: "Cannot determine client IP" };
  }
...
export function signOfficePass(payload: object, ttlSeconds: number): string {
...
export function verifyOfficePass(token: string): boolean {
  const secret = process.env.OFFICE_PASS_SECRET ?? "";
