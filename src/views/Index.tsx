...
type DbEmployee = {
  ...
  weekend_days?: (string | number)[] | null;
};
...
const mapDbEmployee = (dbEmployee: DbEmployee): Employee => ({
  ...
  weekendDays: dbEmployee.weekend_days?.map(d => typeof d === 'string' ? parseInt(d, 10) : d) || undefined,
});
  ...
  const fetchEmployees = useCallback(async (date?: string) => {
    try {
      ...
      const mappedEmployees = employeesData?.map((emp) => {
        ...
        const employee = mapDbEmployee(emp as DbEmployee);
